...
class BuildTest < ActiveSupport::TestCase
  ...
  def test_current_errors_returns_array_of_errors
    project = create_project_stub('one', 'success')
    build = Build.new(project, 1)
    build.stubs(:output).returns(green_build_log)
    assert_equal [], build.current_errors

    build.stubs(:output).returns(red_build_log)
    assert_equal 2, build.current_errors.size

    assert_equal(<<-EOS.strip, build.current_errors[0])
'Shared::CartService::LocalCart persist! should raise an error if the put call fails' FAILED
expected: {:status=>"error", :status_data=>{:error=>"There was a problem saving your cart.  Please try again."}, :reservations=>[]},
     got: {:status=>"error", :status_data=>{:error=>"There was a problem saving your cart.  Please try again."}, :reservations=>[1, 2, 3]} (using ==)
./spec/lib/shared/cart_service/local_cart_spec.rb:360:
    EOS

    assert_equal(<<-EOS.strip, build.current_errors[1])
'Warehouse::HandheldFile ::job_process_all for merchant transaction files handles exceptions during processing by saving the backtrace' FAILED
expected no RuntimeError, got #<RuntimeError: RuntimeError>
./spec/models/warehouse/handheld_file_spec.rb:64:
    EOS
  end

  def test_changeset_emails_returns_emails_from_committers
    project = create_project_stub('one', 'failure')
    build = Build.new(project, 1)
    build.stubs(:changeset).returns(changeset_log)
    assert_equal build.changeset_emails.sort, ['elorenzo@gilt.com', 'jknowles@gilt.com', 'mdemanett@gilt.com']
  end
  ...
  def test_has_new_errors?
    project = create_project_stub('one', 'success')
    build = Build.new(project, 2)

    build.stubs(:new_errors).returns([])
    assert_false build.has_new_errors?

    build.stubs(:new_errors).returns(['one', 'two'])
    assert build.has_new_errors?
  end

  def test_new_errors
    with_sandbox_project do |sandbox, project|
      sandbox.new :file => "build-2-success.in9.235s/build.log", :with_content => red_build_log
      sandbox.new :file => "build-1-success.in9.235s/build.log", :with_content => previous_red_build_log

      current_build  = Build.new(project, 2)
      previous_build = Build.new(project, 1)

      assert_equal 2, current_build.new_errors.size
    end
  end
